...
- [ ] T001 [P] Zod schemas for export route params — `listIdSchema` (enum of the 7 fixed list IDs) and
...
- [ ] T002 Author migration `0011_icontact_export.sql` (`mailing_list_id` enum: `contra, english,
...
- [ ] T003 [P] Drizzle schema for `mailing_list_exports` (+ `mailing_list_id` enum) in
  `src/server/db/schema/mailingListExports.ts`; export from `src/server/db/schema/index.ts`
- [ ] T004 Apply migration to dev DB and extend `resetDb` to truncate `mailing_list_exports` in
  `tests/integration/helpers/db.ts`
- [ ] T005 [P] CSV serialization helpers `toCsvField` (quote on comma/quote/newline, double internal
  quotes) and `rowsToCsv` (header row + escaped rows) in `src/server/domain/exports/csv.ts` (research
  Decision 1)
- [ ] T006 Export-audit helpers in `src/server/domain/exports/exportAuditService.ts`:
...
- [ ] T007 [P] [US1] Unit test: `toCsvField`/`rowsToCsv` quote fields containing a comma, a quote (with
  internal quotes doubled), or a newline, and leave plain fields unquoted, in
  `tests/unit/exports.csv.test.ts`
- [ ] T008 [P] [US1] Unit test: `displayName` → first/last name split (last whitespace boundary =
  last name, remainder = first name; single-word name → last name only) in
  `tests/unit/exports.nameSplit.test.ts` (FR-011)
- [ ] T009 [P] [US1] Integration test: topic-list qualification for contra/english/openband/
...
- [ ] T010 [P] [US1] Integration test: member-list qualification — a contact with `list_member = true`
...
- [ ] T011 [P] [US1] Integration test: performer-list qualification — any contact referenced by a
...
- [ ] T012 [P] [US1] Integration test: a contact with two qualifying emails on the same list produces
  two CSV rows, never collapsed to one, in `tests/integration/exports.multiEmail.test.ts` (edge case,
  Decision 5)
- [ ] T013 [P] [US1] Integration test: `GET /api/exports/:listId` returns `Content-Disposition:
...
- [ ] T014 [P] [US1] Integration test: `GET /api/exports` returns all 7 lists with `lastExport: null`
...
- [ ] T015 [US1] Static list registry (7 entries: id → kind `topic`/`derived`, consent topic where
  applicable) in `src/server/domain/exports/mailingLists.ts`
- [ ] T016 [US1] `buildListRows(db, listId)` in `src/server/domain/exports/exportService.ts` — topic-
...
- [ ] T017 [US1] Jane Austen Ball "most recent year" lookup (latest `events.event_date` whose
  `event_groups.kind = 'jane_austen_ball'`) in `src/server/domain/exports/exportService.ts`
  (Decision 6)
- [ ] T018 [US1] Route handler `GET /api/exports` (7-list metadata + `lastExport` via
  `getLastExports` + JAB `note`) in `src/app/api/exports/route.ts`
- [ ] T019 [US1] Route handler `GET /api/exports/[listId]` — validate `listId` (`listIdSchema`), build
...
- [ ] T020 [US1] Admin exports page — table of the 7 lists with last-exported info and a per-list
...
- [ ] T021 [P] [US2] Unit test: through-year derivation returns the calendar year of the given expiry
  date (and `null` when no expiry is given) in `tests/unit/exports.throughYear.test.ts` (FR-007)
- [ ] T022 [P] [US2] Integration test: member.csv rows include `membership_through_year` equal to the
...
- [ ] T023 [US2] Through-year pure function (`expiryDate: string | null → number | null`) in
  `src/server/domain/exports/throughYear.ts`
- [ ] T024 [US2] Wire `membership_through_year` into the member-list branch of `buildListRows` — join
...
- [ ] T025 [P] [US3] Integration test: contact-tracing qualification — an attendee with an active email
...
- [ ] T026 [P] [US3] Integration test: `GET /api/events?from=<today − 90 days>` excludes an event whose
...
- [ ] T027 [P] [US3] Integration test: `GET /api/exports/contact-tracing?eventId=` returns `{ count: 0
...
- [ ] T028 [US3] `buildContactTracingRows(db, eventId)` in
...
- [ ] T029 [US3] Route handler `GET /api/exports/contact-tracing` — validate `eventId`
...
- [ ] T030 [US3] Admin exports page — contact-tracing section: event dropdown sourced from
...
- [ ] T031 [P] Update the dev route index `src/app/dev/routes/page.tsx` with the new routes (UI
  `/exports`; API `/api/exports`, `/api/exports/[listId]`, `/api/exports/contact-tracing`) per the
  temporary convention (CLAUDE.md)
- [ ] T032 [P] Seed sample contacts/emails covering each consent topic, plus a `jane_austen_ball`
  event group, in `src/server/db/seed.ts` (aids manual testing per quickstart.md)
- [ ] T033 [P] Verify all [quickstart.md](quickstart.md) scenarios end-to-end
- [ ] T034 [P] Constitution compliance pass: strict types, no undocumented `any`/`as`, structured
