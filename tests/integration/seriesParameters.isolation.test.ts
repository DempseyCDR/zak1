...
describe("series-scoped rate isolation", () => {
  ...
  it("a rate set for one series has no effect on another series", async () => {
    ...
    await createRateParameter(db, { seriesKey: "ecd", kind: "caller", amount: 120, effectiveDate: "2026-01-01" });
    ...
    const communityDance = await db.query.series.findFirst({ where: eq(series.key, "community_dance") });

    expect(
      await resolveParameterCents(db, { category: "rate", kind: "caller", seriesId: tnc!.id, onDate: "2026-06-18" }),
    ).toBe(15000);
    expect(
      await resolveParameterCents(db, { category: "rate", kind: "caller", seriesId: ecd!.id, onDate: "2026-06-18" }),
    ).toBe(12000);
