...
import { createEvent, createEventGroup, updateEventDetails } from "@/server/domain/events/eventService";
...
describe("event label", () => {
  ...
  it("distinguishes two same-group, same-day events by label; a rename applies", async () => {
    ...
    const a = await createEvent(db, {
      seriesKey: "tnc", eventDate: "2026-06-20", chargesAdmission: true, groupId: group.id, label: "Afternoon",
    });
    const b = await createEvent(db, {
      seriesKey: "tnc", eventDate: "2026-06-20", chargesAdmission: true, groupId: group.id, label: "Evening",
    });
  ...
  it("an event with no label yields null", async () => {
    const e = await createEvent(db, { seriesKey: "tnc", eventDate: "2026-06-21", chargesAdmission: true });
    const sched = await getPublicSchedule(db, "2026-01-01");
