...
describe("fuzzy name search", () => {
  ...
  it("returns ranked matches for a partial name", async () => {
    await db.insert(contacts).values([
      { displayName: "Ada Lovelace", nameNormalized: normalizeName("Ada Lovelace") },
      { displayName: "Adam Smith", nameNormalized: normalizeName("Adam Smith") },
      { displayName: "Grace Hopper", nameNormalized: normalizeName("Grace Hopper") },
    ]);
