...
describe("getPublicEventDetail", () => {
  ...
  it("applies public-display rules, hides Sound Tech, groups bands, and leaks no private data", async () => {
    ...
    await db
      .update(performers)
      .set({ bio: "Calls contras.", photoUrl: "https://ex.com/cal.jpg" })
      .where(eq(performers.id, caller.id));
    ...
    await createBooking(db, evt.id, {
      performerId: soundTech.id,
      performerType: "sound_tech",
      pay: 100,
    });
    await createBooking(db, evt.id, {
      performerId: openBand.id,
      performerType: "open_band_musician",
    });
    await createBooking(db, evt.id, {
      performerId: instructor.id,
      performerType: "instructor",
      note: "Teaches the beginner lesson",
    });
    await createBooking(db, evt.id, {
      performerId: adHocMusician.id,
    ...
    expect(detail!.bandBlocks[0]).toMatchObject({
      name: "The Rovers",
      bio: "A touring band.",
      photoUrl: "https://ex.com/rovers.jpg",
    });
    ...
    const callerEntry = detail!.performers.find(
      (p) => p.kind === "full_bio" && "name" in p && p.name === "Cal Caller",
    );
    expect(callerEntry).toMatchObject({
      name: "Cal Caller",
      bio: "Calls contras.",
      photoUrl: "https://ex.com/cal.jpg",
    });

    const instructorEntry = detail!.performers.find((p) => p.kind === "name_note");
    expect(instructorEntry).toMatchObject({
      name: "Ivy Instructor",
      note: "Teaches the beginner lesson",
    });
