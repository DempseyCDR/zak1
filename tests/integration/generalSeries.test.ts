...
import { createRateParameter, resolveParameterCents } from "@/server/domain/parameters/seriesParameterService";
...
describe("general series", () => {
  ...
  it("exists after migration and resolves a rate scoped to it", async () => {
    ...
    await createRateParameter(db, { seriesKey: "general", kind: "caller", amount: 175, effectiveDate: "2026-01-01" });
    expect(
  ...
  it("does not fall back from a standing series to general, or vice versa", async () => {
    ...
    expect(
      await resolveParameterCents(db, { category: "rate", kind: "caller", seriesId: general!.id, onDate: "2026-06-18" }),
    ).toBe(0);

    // Rate set for general only.
    await createRateParameter(db, { seriesKey: "general", kind: "sound_tech", amount: 90, effectiveDate: "2026-01-01" });
    expect(
